...
describe("trading rules", () => {
  it("parses the title, update date, and all rule sections", () => {
    ...
    expect(tradingRules.rules).toHaveLength(8);
  ...
  it("keeps multiline rule copy intact while ignoring separators", () => {
  ...
  it("renders every rule and the required-reading warning", () => {
    ...
    expect(wrapper.findAll(".trading-rule")).toHaveLength(8);
